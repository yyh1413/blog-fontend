...
import Files from '../../compoents/files';
import GS from '../../compoents/gs';
...
const Index: FC = () => {
	...
	return (
		<View className='fund_holde_info_box'>
			...
			<View className='charts'>
				<Tabs value={tab1value} onChange={({ paneKey }) => {
					...
					<Tabs.TabPane title="净值估算" className='tab_pane'>
						<View className='dis_aj jz_img'>
							<Image src={`http://j4.dfcfw.com/charts/pic6/${code}.png`} />
						</View>
					</Tabs.TabPane>
				</Tabs>
			</View>
			...
			<View className='files'><Files code={code!} ENDNAV={data?.ENDNAV} ESTABDATE={data?.ESTABDATE} /></View>
			{data?.JJGSID && <View className='gs'><GS code={data?.JJGSID} /></View>}
		</View >
	)
}
